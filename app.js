...
function processTagDisplayLabel(tag) {
  return PROCESS_TAG_DISPLAY_LABELS[tag] || tag;
}

function canonicalAiMaterialName(value) {
  const text = String(value || "").trim();
  if (!text) return "";
  const compact = text.replace(/[\s　_\-－—–/／()（）【】\[\]]+/g, "").toLowerCase();
  if (/壓克力|压克力|亞克力|亚克力/.test(compact)) return "壓克力";
  if (/^pc$|聚碳酸酯/.test(compact)) return "PC";
  if (/pvc/.test(compact)) return "PVC";
  if (/安迪板|發泡板|发泡板/.test(compact)) return "安迪板";
  if (/^pet$/.test(compact)) return "PET";
  if (/貼紙|贴纸|透貼|透贴|銀貼|银贴|地板貼|地板贴/.test(compact)) return "貼紙";
  if (/3d膜/.test(compact)) return "3D膜";
  if (/燈條|灯条/.test(compact)) return "燈條";
  if (/立體字|立体字|發光字|发光字/.test(compact)) return "立體字";
  if (/其他/.test(compact)) return "其他";
  return "";
}

function getAiMaterialMenu() {
  const derived = STANDARD_MATERIAL_OPTIONS.map(canonicalAiMaterialName).filter(Boolean);
  return [...new Set([...derived, "其他"])];
}
...
/**
 * 將 Gemini 回傳的尺寸／厚度字串清洗成 mm 純數字。
 * - 直接數字與 mm：原值
 * - cm／公分／厘米：× 10
 * - m／公尺：× 1000
 * - inch／吋：× 25.4
 */
function parseAiMeasurementMm(value) {
  ...
  const unitText = compact.toLowerCase();
  if (/(公分|厘米|釐米|cm)/i.test(unitText)) amount *= 10;
  else if (/(公尺|meter|metre)/i.test(unitText) || /(^|[^m])m($|[^m])/i.test(unitText)) amount *= 1000;
  else if (/(英吋|英寸|吋|inch|in)/i.test(unitText)) amount *= 25.4;
  return Math.max(0, Math.round(amount * 10000) / 10000);
}

function aiProcessSourceValues(aiItem) {
  const values = [];
  const append = (value) => {
  ...
  append(aiItem?.processes);
  append(aiItem?.processTags);
  append(aiItem?.processSummary);
  append(aiItem?.printMethod);
  append(aiItem?.printSide);
  append(aiItem?.whiteInk);
  append(aiItem?.specialEffect);
  append(aiItem?.constraints);
...
/**
 * Gemini JSON 進入任何前端 state 前的第二層防禦清洗。
 * 保留原始欄位，並輸出可直接供既有 mapping／data-binding 使用的同名欄位。
 */
function normalizeAIExtractedItem(aiItem) {
  const source = aiItem && typeof aiItem === "object" ? aiItem : {};
  const originalMaterial = String(source.originalMaterial || source.material || source.normalizedMaterial || "").trim();
  const materialText = originalMaterial.replace(/[\s　_\-－—–/／()（）【】\[\]]+/g, "");
  const canonicalMaterial = canonicalAiMaterialName(originalMaterial);
  let material = "";
  if (/壓克力|压克力|亞克力|亚克力/i.test(materialText)) material = "壓克力";
  else if (/(^|[^a-z])pc([^a-z]|$)|聚碳酸酯/i.test(materialText)) material = "PC";
  else if (/pvc/i.test(materialText)) material = "PVC";
  else if (/安迪板|發泡板|发泡板/i.test(materialText)) material = "安迪板";
  else if (/貼紙|贴纸|透貼|透贴|銀貼|银贴|地板貼|地板贴/i.test(materialText)) material = "貼紙";
  else material = canonicalMaterial || String(source.normalizedMaterial || source.material || "").trim();

  const widthMm = parseAiMeasurementMm(source.widthMm ?? source.width);
  const heightMm = parseAiMeasurementMm(source.heightMm ?? source.height);
  const thicknessMm = parseAiMeasurementMm(source.thicknessMm ?? source.thickness);
  const existingMissingFields = Array.isArray(source._missingFields) ? source._missingFields : [];
  const missingFields = [...existingMissingFields];
  // 材質有原文但完全無法收斂到系統選項時，保留原文供人工判讀並標記低信心。
  if (!canonicalMaterial) missingFields.push("material");
  if (!(widthMm > 0)) missingFields.push("widthMm");
  if (!(heightMm > 0)) missingFields.push("heightMm");
  const normalizedMissingFields = unique(missingFields.filter(Boolean));
  const originalProcesses = aiProcessSourceValues(source);
  const normalizedProcesses = unique(originalProcesses.map(fuzzyNormalizeAiProcessTag).filter(Boolean));
  const rejectedProcesses = originalProcesses.filter((tag) => !fuzzyNormalizeAiProcessTag(tag));

  const dimensions = Array.isArray(source.dimensions)
    ? source.dimensions.map((row) => ({
        ...row,
        widthMm: parseAiMeasurementMm(row?.widthMm ?? row?.width),
        heightMm: parseAiMeasurementMm(row?.heightMm ?? row?.height),
        qty: Math.max(1, parseFloat(String(row?.qty ?? 1).replace(/[^\d.+-]/g, "")) || 1),
      })).filter((row) => row.widthMm > 0 && row.heightMm > 0)
    : [];

  const generatedSpec = widthMm > 0 && heightMm > 0 ? `${widthMm}×${heightMm}mm` : "";
  const noteParts = [String(source.note || "").trim()];
  if (rejectedProcesses.length) noteParts.push(`AI 未對應製程：${unique(rejectedProcesses).join("、")}`);
  ...
  return {
    ...
    heightMm,
    thicknessMm,
    thickness: thicknessMm > 0 ? String(thicknessMm) : String(source.thickness || "").trim(),
    dimensions,
    spec: String(source.spec || generatedSpec).trim(),
    originalProcesses,
    processes: normalizedProcesses,
    processTags: normalizedProcesses,
    processSummary: normalizedProcesses.join("、"),
    note: noteParts.filter(Boolean).join("；"),
    _aiConfidenceLow: Boolean(source._aiConfidenceLow) || normalizedMissingFields.length > 0,
    _missingFields: normalizedMissingFields,
    _aiUnmappedMaterial: !canonicalMaterial && originalMaterial ? originalMaterial : "",
  };
}

function aiReviewMissingLabel(field) {
  return ({ material: "材質", widthMm: "寬度", heightMm: "高度", dimension: "尺寸" })[field] || field;
}

function refreshDraftAiConfidence(item) {
  if (!item?.aiImported || item.itemType === "附加費用") return;
  const missing = [];
  const material = String(item.material || "").trim();
  const dimensions = parseDimensions(String(item.spec || ""));
  if (!material) missing.push("material");
  if (!(toNumber(dimensions.widthMm) > 0)) missing.push("widthMm");
  if (!(toNumber(dimensions.heightMm) > 0)) missing.push("heightMm");
  item._missingFields = unique(missing);
  item._aiConfidenceLow = item._missingFields.length > 0;
}

function refreshEstimateAiConfidence(item) {
  if (!item?.aiImported) return;
  const missing = [];
  if (!String(item.material || "").trim()) missing.push("material");
  if (isLightStripItem(item)) {
    if (!(toNumber(item.lightStripTotalLengthM) > 0)) missing.push("dimension");
  } else {
    if (!(toNumber(item.widthMm) > 0)) missing.push("widthMm");
    if (!(toNumber(item.heightMm) > 0)) missing.push("heightMm");
  }
  item._missingFields = unique(missing);
  item._aiConfidenceLow = item._missingFields.length > 0;
}

function updateDraftAiReviewRow(row, item) {
  if (!row || !item) return;
  refreshDraftAiConfidence(item);
  const missing = new Set(item._missingFields || []);
  const needsReview = Boolean(item._aiConfidenceLow);
  row.classList.toggle("ai-review-row", needsReview);
  row.classList.toggle("error-highlight", needsReview);
  row.querySelector('[data-ai-review-badge]')?.toggleAttribute('hidden', !needsReview);
  row.querySelector('[data-ai-material-cell]')?.classList.toggle('ai-review-field', missing.has('material'));
  row.querySelector('[data-ai-spec-cell]')?.classList.toggle('ai-review-field', missing.has('widthMm') || missing.has('heightMm') || missing.has('dimension'));
}

function updateEstimateAiReviewState(row, item) {
  if (!row || !item) return;
  refreshEstimateAiConfidence(item);
  const needsReview = Boolean(item._aiConfidenceLow);
  row.classList.toggle('ai-review-card', needsReview);
  row.querySelector('[data-ai-review-badge]')?.toggleAttribute('hidden', !needsReview);
}
...
// 最低開機費只對「整批」套用一次，不得乘上品項數量。
const MINIMUM_CHARGE = Object.freeze({
  '壓克力': 200,
  'PC': 150,
  'PVC': 150,
  '貼紙': 50,
...
const ESTIMATE_PRICING_CONFIG = Object.freeze({
  ...
  // 舊函式與 data-binding 使用百分比，數值由 WASTE_RATE 統一轉換。
  MATERIAL_WASTE_RATE: Object.freeze({
    '壓克力': WASTE_RATE['壓克力'] * 100,
    'PC': WASTE_RATE.PC * 100,
    'PVC': WASTE_RATE.PVC * 100,
    '貼紙': WASTE_RATE['貼紙'] * 100,
    '安迪板': WASTE_RATE['安迪板'] * 100,
    '3D膜': 10,
    '預設': WASTE_RATE['預設'] * 100,
  }),
});
...
// 保留既有名稱，實際來源統一由 MINIMUM_CHARGE 管理。
...
const FOUR_STAGE_SMALL_START_TSAI = 0.25;
const FOUR_STAGE_NEAR_ONE_START_TSAI = 0.85;
const FOUR_STAGE_STANDARD_START_TSAI = 1.0;
const IRREGULAR_SHAPE_MULTIPLIER = 1.30;
...
function estimateConfidenceMeta(value) {
  const score = Math.max(0, Math.min(100, Math.round(toNumber(value))));
  if (score >= 85) return { score, label: "參考價", className: "confidenceReference" };
  if (score >= 60) return { score, label: "粗估", className: "confidenceRough" };
  return { score, label: "僅供參考", className: "confidenceLowReference" };
}

function estimateHistoryAverage(item) {
  const prices = (item?.similarHistoryRecords || []).map((row) => toNumber(row.unitPrice)).filter((value) => value > 0);
  if (!prices.length) return null;
  return { count: prices.length, average: prices.reduce((sum, value) => sum + value, 0) / prices.length };
}

function estimateSummarySpec(item) {
  const material = String(item?.material || "材質待補").trim();
  const thickness = String(item?.thickness || "").trim();
  const size = isLightStripItem(item)
    ? (toNumber(item.lightStripTotalLengthM) > 0 ? `${roundDisplay(item.lightStripTotalLengthM)}m` : "長度待補")
    : (toNumber(item?.widthMm) > 0 && toNumber(item?.heightMm) > 0 ? `${roundDisplay(item.widthMm)}×${roundDisplay(item.heightMm)}mm` : "尺寸待補");
  return [material, thickness, size, `${Math.max(1, toNumber(item?.qty) || 1)}件`].filter(Boolean).join(" | ");
}

function estimateProcessPillsHtml(value) {
  const tags = normalizeProcessTags(value);
  return tags.length
    ? tags.map((tag) => `<span class="estimateProcessPill">${escapeHTML(processTagDisplayLabel(tag))}</span>`).join("")
    : '<span class="estimateProcessPill empty">尚未選擇製程</span>';
}
...
function estimateWarningMeta(item, aiMissing = new Set()) {
  ...
  if (aiMissing.size) {
    return { kind: "missing", message: `⚠️ 需人工確認：${[...aiMissing].map(estimateAiMissingLabel).join("、")}`, showProcessButton: aiMissing.has("processSummary") };
  }
  if (item?.aiImported && toNumber(item?.confidenceScore) < 60) {
    return { kind: "confidence", message: "⚠️ AI 信心度偏低，請人工確認材質、尺寸與製程", showProcessButton: true };
  }
  return null;
}
...
function applyQuotationAnalysis(result) {
  ...
  const rows = [
    ...materialItems.map((rawItem) => {
      const item = normalizeAIExtractedItem(rawItem);
      const normalizedSpec = String(item.spec || (item.widthMm > 0 && item.heightMm > 0 ? `${item.widthMm}×${item.heightMm}mm` : "")).trim();
      return {
      itemType: "材料",
      ...
      processTags: processTagsText(item.processes || item.processTags || [item.printMethod, item.printSide, item.whiteInk, item.specialEffect].filter(Boolean)),
      ...
      aiImported: true,
      _aiConfidenceLow: Boolean(item._aiConfidenceLow),
      _missingFields: Array.isArray(item._missingFields) ? [...item._missingFields] : [],
      _aiUnmappedMaterial: String(item._aiUnmappedMaterial || ""),
    };
    }),
    ...feeItems.map((fee) => ({
      ...emptyDraftItem("附加費用"),
...
function handleDraftRowInput(event) {
  ...
  if (field === "itemType" && item.itemType === "附加費用") {
    ...
    item.image = null;
    renderDraftItems();
    // itemType 切換會重建整列 DOM；重建後必須立即重算總計，避免總金額停留在舊值。
    renderDraftSubtotal();
    return;
  }
...
async function handleCreateCostOrder(event) {
  ...
  try {
    ...
    const result = response.result || {};
    const firstMaterial=validItems.find((item)=>item.itemType!=='附加費用')||validItems[0];
...
function renderDraftItems() {
  ...
  $("draftItemRows").innerHTML = state.draftItems.map((item, index) => {
    ...
    const aiMissing = new Set(item._missingFields || []);
    const needsAiReview = !isFee && Boolean(item._aiConfidenceLow);
    const reviewText = aiMissing.size
      ? `需確認：${[...aiMissing].map(aiReviewMissingLabel).join('、')}`
      : '需人工確認';
    return `<tr data-index="${index}" class="${isFee ? "feeRow" : "materialRow"} ${needsAiReview ? 'ai-review-row error-highlight' : ''}">
...
function normalizeEstimateItem(row) {
  return {
    ...
    processSummary: [
      String(firstValue(row, ["印刷方式", "printMethod"])),
      String(firstValue(row, ["印刷面", "printSide"])),
      String(firstValue(row, ["白墨範圍", "whiteInk"])),
      String(firstValue(row, ["特殊效果", "specialEffect"])),
      String(firstValue(row, ["不可變更條件", "constraints"]))
    ].filter(Boolean).join('、'),
    aiImported: false,
    aiMissingFields: [],
    _aiConfidenceLow: false,
...
function setupV20() {
  ...
  $('quickEstimateAiScanFile')?.addEventListener('change', handleQuickEstimateAiScanFile);
  $('quickEstimateAiScan')?.addEventListener('click', () => $('quickEstimateAiScanFile')?.click());
  setupEstimateUploadDropZone();
  ...
  $('estimateDeveloperMode')?.addEventListener('change', (event) => { document.body.classList.toggle('estimate-developer-mode', event.target.checked); });
  ...
  $('estimateSaveSettingsForm')?.addEventListener('submit', handleEstimateSaveSettingsSubmit);
  $('cancelEstimateSaveSettings')?.addEventListener('click', ()=>closeEstimateSaveSettingsDialog(false));
  $('closeEstimateSaveSettings')?.addEventListener('click', ()=>closeEstimateSaveSettingsDialog(false));
  $('estimateSaveSettingsDialog')?.addEventListener('cancel',(event)=>{event.preventDefault();closeEstimateSaveSettingsDialog(false);});
  $('estimateSaveSettingsDialog')?.addEventListener('click',(event)=>{if(event.target===$('estimateSaveSettingsDialog'))closeEstimateSaveSettingsDialog(false);});
  $('toggleEstimateTextImport')?.addEventListener('click',()=>{
    const panel=$('estimateTextImportDetails');
    if(panel){panel.open=!panel.open;if(panel.open)setTimeout(()=>$('estimateTextInput')?.focus(),0);}
  });
  $('quickManualEstimate')?.addEventListener('click',()=>{
    state.estimateDraftItems.push(emptyEstimateItem());
    renderEstimateDraft();
    setTimeout(()=>{
      const workspace=$('estimateWorkspacePanel');
      workspace?.scrollIntoView({behavior:'smooth',block:'start'});
      $('estimateItemRows')?.querySelector('[data-estimate-field="name"]')?.focus();
    },0);
  });
  ...
  if (!$('estimateOrderType').value) $('estimateOrderType').value = 'auto';
  updateEstimateTargetMode();
...
function extractHistoricalItemCode(value){
  const text=String(value||'').toUpperCase().replace(/[－—–]/g,'-');
  const match=text.match(/(?:^|[^A-Z0-9])([A-Z]{1,5})\s*[-_ ]?\s*(\d{1,4})(?=$|[^A-Z0-9])/);
  return match?`${match[1]}${match[2]}`:'';
}

function compactHistoricalItemName(value){
  return standardizeErpText(value||'')
    .toLowerCase()
    .replace(/透明|乳白|冬瓜白|霧面|黑色|鏡面/g,'')
    .replace(/壓克力|亞克力|pvc|pc|安迪板|貼紙/g,'')
    .replace(/[\s　_\-－—–/／()（）【】\[\],，.。:：]+/g,'');
}

function exactHistoricalQuoteScore(review,item){
  if(!review||!(toNumber(review.unitPrice)>0))return-999;
  if(!strictMaterialCompatibility(item.material||item.name,review.material))return-999;

  const itemThickness=numericThickness(item.thickness||item.spec);
  const reviewThickness=numericThickness(review.thicknessMm||review.thickness);
  const descriptor=materialDescriptor(item.material||review.material);
  if(descriptor.requiresThickness){
    if(!(itemThickness>0&&reviewThickness>0&&Math.abs(itemThickness-reviewThickness)<=0.11))return-999;
  }

  const iw=toNumber(item.widthMm),ih=toNumber(item.heightMm);
  const rw=toNumber(review.widthMm),rh=toNumber(review.heightMm);
  if(!(iw>0&&ih>0&&rw>0&&rh>0))return-999;
  const direct=Math.max(Math.abs(iw-rw)/Math.max(rw,1),Math.abs(ih-rh)/Math.max(rh,1));
  const rotated=Math.max(Math.abs(iw-rh)/Math.max(rh,1),Math.abs(ih-rw)/Math.max(rw,1));
  const dimensionDiff=Math.min(direct,rotated);
  if(dimensionDiff>0.035)return-999;

  const targetCode=extractHistoricalItemCode([item.code,item.name].filter(Boolean).join(' '));
  const reviewCode=extractHistoricalItemCode([review.itemCode,review.itemName].filter(Boolean).join(' '));
  const targetName=compactHistoricalItemName(item.name||item.itemName);
  const reviewName=compactHistoricalItemName(review.itemName);
  const codeExact=Boolean(targetCode&&reviewCode&&targetCode===reviewCode);
  const nameExact=Boolean(targetName&&reviewName&&(targetName===reviewName||targetName.includes(reviewName)||reviewName.includes(targetName)));
  // 待認證資料只在「同品號／同名稱 + 同材質厚度尺寸」時可作精確實價，避免污染一般公式。
  if(!codeExact&&!nameExact)return-999;

  let score=120;
  if(codeExact)score+=80;
  if(nameExact)score+=45;
  if(itemThickness>0&&reviewThickness>0)score+=20;
  if(dimensionDiff<=0.01)score+=45;
  else if(dimensionDiff<=0.03)score+=30;

  const targetTags=new Set(normalizeProcessTags(extractEstimateProcessTags(item)).map(norm));
  const reviewTags=new Set(normalizeProcessTags(review.processTags||'').map(norm));
  if(targetTags.size&&reviewTags.size){
    const overlap=[...targetTags].filter((tag)=>reviewTags.has(tag)).length;
    score+=Math.min(25,overlap*5);
  }
  if(review.status==='已認證'&&review.includeBaseline==='是')score+=15;
  return score;
}
...
function bestExactHistoricalQuoteForItem(item){
  ...
  const best=candidates[0];
  const peers=candidates.filter((entry)=>entry.score>=best.score-5).slice(0,5);
  const prices=peers.map((entry)=>toNumber(entry.row.unitPrice)).filter((value)=>value>0);
  if(!prices.length)return null;
  const approved=best.row.status==='已認證'&&best.row.includeBaseline==='是';
  return{
    unitPrice:medianNumber(prices),
    score:best.score,
    count:peers.length,
...
async function prepareEstimateSourceFile(file) {
  if (!file) return;
  try {
    ...
    setEstimateAiStatus(`${state.estimateDocumentPayload.mimeType === 'application/pdf' ? 'PDF' : '圖片'}已準備完成，可進行 AI 掃描。`, 'ready');
  } catch (error) {
    clearEstimateSourceDocument();
    showNotice(`快速估價檔案處理失敗：${error.message}`, 'error');
  }
}

async function handleEstimateSourceFile(event) {
  await prepareEstimateSourceFile(event.target.files?.[0]);
  if(state.estimateDocumentPayload)await analyzeEstimateSourceDocument();
}

async function handleQuickEstimateAiScanFile(event) {
  const input = event.target;
  const file = input?.files?.[0];
  if (!file) return;
  const shortcutButton = $('quickEstimateAiScan');
  const originalText = shortcutButton?.textContent || '🤖 AI 掃描報價單/圖片自動填入';
  try {
    if (shortcutButton) {
      shortcutButton.disabled = true;
      shortcutButton.textContent = 'AI 檔案處理中…';
    }
    await prepareEstimateSourceFile(file);
    if (!state.estimateDocumentPayload) return;
    await analyzeEstimateSourceDocument();
  } finally {
    if (input) input.value = '';
    if (shortcutButton) {
      shortcutButton.disabled = false;
      shortcutButton.textContent = originalText;
    }
  }
}

function setupEstimateUploadDropZone() {
  const zone = document.querySelector('.estimateUploadZone');
  if (!zone || zone.dataset.dropReady === '1') return;
  zone.dataset.dropReady = '1';
  ['dragenter','dragover'].forEach((name)=>zone.addEventListener(name,(event)=>{
    event.preventDefault();
    zone.classList.add('isDragging');
  }));
  ['dragleave','drop'].forEach((name)=>zone.addEventListener(name,(event)=>{
    event.preventDefault();
    zone.classList.remove('isDragging');
  }));
...
function clearEstimateSourceDocument() {
  ...
  if ($('quickEstimateAiScanFile')) $('quickEstimateAiScanFile').value = '';
...
function useDocumentPriceForEstimate(item, unitPrice, confidence, fileName) {
  const normalizedUnitPrice=Math.max(0,toNumber(unitPrice));
  item.manualPrice = true;
  item.manualPriceConfidence = normalizeAiConfidence(confidence, 80);
  item.priceId = '';
  item.baseUnitPrice = normalizedUnitPrice;
  item.aiPrice = roundCurrency(normalizedUnitPrice * Math.max(1,toNumber(item.qty)||1));
  item.aiConfidence = item.manualPriceConfidence;
...
function estimateLightStripByDatabase(item,basisKey){
  ...
  return{
    ...
    breakdown:[{label:`${label} ${Math.round(length.totalLengthM*1000)/1000}m`,amount:unitPrice,type:'材料'}],
    missing:[],isCompositePrice:false,isLightStrip:true,
    // 交由 recalculateEstimateItem 套用使用者／預設耗損率，並在分項顯示「含損耗」。
    wasteHandled:false,
  };
}
...
function applyAutomaticEstimatePricingUnit(item){
  const unit=automaticEstimatePricingUnit(item);
  item.pricingUnit=unit;
  const usesWholeItemPrice=Boolean(item?.manualPrice||item?.historicalPriceApplied||item?.productionReferenceApplied);
  item.unit=usesWholeItemPrice?'件':unit;
  return unit;
}

function unifiedQuantityDiscount(qty){
  const count=Math.max(1,toNumber(qty)||1);
...
function summarizeEstimateDimensions(item){
  ...
  // AI 常把「品項數量」同時寫進 item.qty 與 dimensions[0].qty，若兩邊再相乘會平方暴增。
  // 單一尺寸列的 dimensions.qty 視為單件內部數量；當它與整列 qty 完全相同時，判定為重複欄位並歸一為 1。
  const rowQty=Math.max(1,toNumber(item.qty)||1);
  if(groups.length===1&&rowQty>1&&Math.max(1,toNumber(groups[0].qty)||1)===rowQty){
    groups[0]={...groups[0],qty:1,sourceText:`${groups[0].sourceText||'AI尺寸'}｜已移除重複數量`};
  }
...
function estimateProcessSummaryText(item) {
  const tags=extractEstimateProcessTags(item);
  return tags.join('、') || [item.printMethod,item.printSide,item.whiteInk,item.specialEffect,item.constraints].filter(Boolean).join('、');
}
...
function applyEstimateProcessSummary(item, value) {
  ...
  item.printMethod=print.filter((tag)=>!['正面印刷','背面印刷','白色直噴'].includes(tag)).join('＋');
  item.printSide=print.filter((tag)=>['正面印刷','背面印刷'].includes(tag)).join('＋');
  item.whiteInk=print.includes('白色直噴')?'白色直噴':'';
  item.specialEffect=effects.join('＋');
  item.constraints=processes.join('＋');
}

function estimateAiMissingFields(item) {
  if(!item?.aiImported)return[];
  const missing=[];
  if(!String(item.name||'').trim())missing.push('name');
  if(!String(item.material||'').trim())missing.push('material');
  if(materialDescriptor(item.material||item.name).requiresThickness && !(numericThickness(item.thickness)>0))missing.push('thickness');
  if(isLightStripItem(item)){
    if(!(toNumber(item.lightStripTotalLengthM)>0))missing.push('dimension');
  }else if(!(toNumber(item.widthMm)>0&&toNumber(item.heightMm)>0))missing.push('dimension');
  if(!extractEstimateProcessTags(item).length)missing.push('processSummary');
  if(!(toNumber(item.baselineCost)>0||toNumber(item.baseUnitPrice)>0))missing.push('baseUnitPrice');
  return [...new Set(missing)];
}

function refreshEstimateAiMissingFields(item) {
  if(!item?.aiImported)return;
  item.processSummary=estimateProcessSummaryText(item);
  item.aiMissingFields=estimateAiMissingFields(item);
  refreshEstimateAiConfidence(item);
}

function estimateAiMissingLabel(field) {
  return ({name:'品項名稱',material:'材質',thickness:'厚度',dimension:'尺寸',processSummary:'製程',baseUnitPrice:'價格'})[field]||field;
}

function mapAiEstimateItem(raw, strategy, fileName) {
  raw = normalizeAIExtractedItem(raw);
  ...
  const item = {
    ...
    spec,
    thickness: raw.thicknessMm > 0 ? String(raw.thicknessMm) : String(raw.thickness || '').trim(),
    qty,
    ...
    processSummary: Array.isArray(raw.processTags) ? raw.processTags.join('、') : String(raw.processTags || raw.constraints || '').trim(),
    aiImported: true,
    aiMissingFields: [],
    _aiConfidenceLow: Boolean(raw._aiConfidenceLow),
    _missingFields: Array.isArray(raw._missingFields) ? [...raw._missingFields] : [],
    _aiUnmappedMaterial: String(raw._aiUnmappedMaterial || ''),
    isArtItem: raw.isArtItem === false || String(raw.isArtItem) === '否' ? '否' : '是',
    confidenceScore: normalizeAiConfidence(raw.confidence, 60),
  };

  if(item.processSummary)applyEstimateProcessSummary(item,item.processSummary);
  else item.processSummary=estimateProcessSummaryText(item);
  applyEstimateDimensionNormalization(item);
...
async function analyzeEstimateSourceDocument() {
  ...
  const originalText = button.textContent;
  button.disabled = true;
  button.textContent = 'AI 掃描中…';
  $('estimateAiBadge').textContent = '分析中';
  setEstimateAiStatus('AI 正在掃描品項、材質、尺寸、厚度、製程與可見價格，請稍候…', 'loading');
  try {
    const response = await secureApiRequest({
    ...
    if (summary.missing > 0 && $('estimateAutoPriceMissing')?.checked && $('estimatePriceStrategy').value !== 'company_only') {
      ...
      if($('estimateAutoPriceMissing')?.checked && $('estimatePriceStrategy').value !== 'company_only'){
        await estimateMissingPricesWithAi({ automatic:true, forceReference:true, limit:5, silentIfEmpty:true });
      }
      showNotice(
        summary.missing > 0
          ? `AI 已辨識 ${summary.imported} 筆，但仍有 ${summary.missing} 筆缺價。`
          : 'AI 已帶入並完成估價。',
    ...
    setEstimateAiStatus(`AI 掃描失敗：${error.message}`, 'error');
    showNotice(`AI 掃描失敗：${error.message}`, 'error');
  } finally {
    button.disabled = false;
    button.textContent = originalText;
  }
...
function missingEstimateItems(options = {}){
  ...
  return state.estimateDraftItems
    ...
    .filter(({ item }) => {
      ...
      if(forceReference){
        return !(toNumber(item.aiPrice)>0);
      }
...
async function estimateMissingPricesWithAi(options = {}){
  ...
  const missing = missingEstimateItems({ automatic, forceReference, limit: options.limit || (automatic ? 5 : Number.MAX_SAFE_INTEGER) });
  if (!missing.length) {
  ...
  try {
    ...
    estimates.forEach((result) => {
      ...
      const aiRawPrice = Math.max(0, toNumber(result.estimatedUnitPrice));
      if (!item || aiRawPrice <= 0) {
        if (result?.reason) failedNotes.push(String(result.reason));
        return;
      }
      applyEstimateDimensionNormalization(item);
      const aiExactCai=Math.max(0,(toNumber(item.widthMm)*toNumber(item.heightMm))/90000);
      const aiMinSide=Math.min(Math.max(0,toNumber(item.widthMm)),Math.max(0,toNumber(item.heightMm)));
      const aiMaxSide=Math.max(Math.max(0,toNumber(item.widthMm)),Math.max(0,toNumber(item.heightMm)));
      const aiAspectRatio=aiMinSide>0?aiMaxSide/aiMinSide:0;
      const aiFactor=bundleDiscountFactor(unifiedEstimateProcessTags(item),aiExactCai,aiAspectRatio);
      const unitPrice=Math.max(0,aiRawPrice*aiFactor);
      if(forceReference){
        recalculateEstimateItem(item);
    ...
    missing.forEach(({ item, clientIndex }) => {
      item._autoAiPending = false;
      const referenceSucceeded=forceReference&&returnedIndexes.has(clientIndex)&&toNumber(item.aiPrice)>0;
      if (!referenceSucceeded && (!returnedIndexes.has(clientIndex) || !(toNumber(item.baseUnitPrice) > 0 && item.manualPrice))) {
        if(forceReference)return;
...
function defaultWasteRateForMaterial(materialName){
  const descriptor=materialDescriptor(materialName);
  const ratio=WASTE_RATE[descriptor.family]??WASTE_RATE['預設'];
  return Math.round(ratio*10000)/100; // UI 欄位以百分比表示，例如 0.15 → 15
}

function resolvedWasteRatioForItem(item,materialName){
  if(item?.wasteRateManuallySet){
    // wasteRate 的 data-binding 定義為百分比；15 代表 15%。
    return Math.max(0,toNumber(item.wasteRate))/100;
  }
  const descriptor=materialDescriptor(materialName);
  const ratio=WASTE_RATE[descriptor.family]??WASTE_RATE['預設'];
  item.wasteRate=Math.round(ratio*10000)/100;
  return ratio;
}

function batchMinimumChargeForMaterial(materialName,basisKey=internalPriceBasisKey()){
  const descriptor=materialDescriptor(materialName);
  const base=MINIMUM_CHARGE[descriptor.family]??MINIMUM_CHARGE['預設'];
  return unifiedStageRate(base,basisKey);
}

function applyBatchMaterialMinimum(singleMaterialCost,qty,wasteRatio,minimumCharge){
  const safeQty=Math.max(1,toNumber(qty)||1);
  const batchBeforeWaste=Math.max(0,toNumber(singleMaterialCost))*safeQty;
  const batchWithWaste=batchBeforeWaste*(1+Math.max(0,toNumber(wasteRatio)));
  const finalBatchMaterial=Math.max(batchWithWaste,Math.max(0,toNumber(minimumCharge)));
  return{
    batchBeforeWaste,
    batchWithWaste,
    finalBatchMaterial,
    unitMaterialCost:finalBatchMaterial/safeQty,
    minimumApplied:finalBatchMaterial>batchWithWaste+0.000001,
  };
}
...
function extractEstimateProcessTags(item){
  const text=standardizeErpText([item.processSummary,item.printMethod,item.printSide,item.whiteInk,item.specialEffect,item.constraints,item.spec,item.originalSpec,item.name].filter(Boolean).join(' '));
  const tags=[];
...
function unifiedMaterialKey(item){
  const materialName=canonicalInternalMaterial(item?.material||item?.name||'');
  const descriptor=materialDescriptor(materialName);
  let thickness=numericThickness(item?.thickness);
  if(!(thickness>0)){
    if(descriptor.family==='壓克力')thickness=3;
    else if(descriptor.family==='PVC')thickness=1;
    else if(descriptor.family==='安迪板')thickness=5;
  }
...
function bundleDiscountFactor(processTags, exactCai = 0, aspectRatio = 0) {
  const tags = unique((Array.isArray(processTags) ? processTags : [...(processTags || [])])
    .map((tag) => canonicalProcessTag(tag))
    .filter(Boolean));
  const displays = tags.map((tag)=>processTagDisplayLabel(tag));
  const hasEngraving = tags.includes('雕刻') || displays.includes('雕刻');
  const hasPrinting = tags.some((tag)=>['四色直噴','白色直噴','四色黑','黑色直噴'].includes(tag))
    || displays.some((tag)=>['四色直噴','白色直噴','四色黑','黑色直噴'].includes(tag));
  const hasCutting = tags.some((tag)=>['裁切外型','切割外型'].includes(tag)) || displays.includes('切割外型');
  const onlyCutting = hasCutting && !hasPrinting && tags.every((tag)=>{
    const display=processTagDisplayLabel(tag);
    return ['裁切外型','切割外型','背膠'].includes(tag)||['切割外型','背膠'].includes(display);
  });
  const hasBending = tags.includes('壓克力折彎') || displays.includes('折彎');

  if (hasEngraving) return 1.0;
  if (Math.max(0,toNumber(exactCai)) < 0.25) return 0.35;
  if (Math.max(0,toNumber(aspectRatio)) > 8) return 0.50;
  if (onlyCutting) return 0.40;
  if (hasBending) return 0.65;
  return 0.70;
}
...
function estimateByUnifiedPricingModel(item) {
  ...
  const widthMm=Math.max(0,toNumber(item?.widthMm));
  const heightMm=Math.max(0,toNumber(item?.heightMm));
  const exactCai=(widthMm*heightMm)/90000;
  const billingCai=Math.max(1,Math.ceil(exactCai||0));
  const minSide=Math.min(widthMm,heightMm);
  const maxSide=Math.max(widthMm,heightMm);
  const aspectRatio=minSide>0?maxSide/minSide:0;
  const isTiny=exactCai<0.25;
  const isSmall=exactCai>=0.25&&exactCai<1;
  const isStandard=exactCai>=1;
  const isLongStrip=aspectRatio>8;
  const sizeTier=isTiny?'極小件':isSmall?'小件':'標準件';

  if(!(exactCai>0))item.estimateFailReason='尺寸未填或解析失敗';

  let rawMaterialTwd=toNumber(UNIFIED_RAW_MATERIAL_PER_TSAI[material.key]);
  let materialRateSource='材料基價表';
  if(!(rawMaterialTwd>0)){
    if(material.descriptor.family==='壓克力')rawMaterialTwd=150;
    else if(material.descriptor.family==='PVC')rawMaterialTwd=100;
    else if(material.descriptor.family==='安迪板')rawMaterialTwd=40;
    else if(material.descriptor.family==='PC')rawMaterialTwd=150;
    else if(String(material.name||'').trim())rawMaterialTwd=150;
    else rawMaterialTwd=0;
    materialRateSource=rawMaterialTwd>0?'預設材料基價':'未識別材質';
    if(!item.estimateFailReason)item.estimateFailReason='材質或厚度無法識別';
  }

  const materialRate=rawMaterialTwd;
  const baseWasteRate=Math.max(0,resolvedWasteRatioForItem(item,material.name));
  const appliedWasteRate=exactCai<1?0:exactCai<5?baseWasteRate*0.5:baseWasteRate;
  const singleMaterialCost=exactCai>0&&materialRate>0
    ?exactCai*materialRate*(1+appliedWasteRate)
    :0;

  const processTags=unifiedEstimateProcessTags(item);
  const canonicalTags=processTags.map((tag)=>canonicalProcessTag(tag)).filter(Boolean);
  const displayTags=canonicalTags.map((tag)=>processTagDisplayLabel(tag));
  const hasBending=canonicalTags.includes('壓克力折彎')||displayTags.includes('折彎');
  const breakdown=[];
  const missing=[];
  let singleProcessCost=0;

  if(singleMaterialCost>0){
    breakdown.push({
      label:`${material.name||'材料'}｜${roundDisplay(exactCai)}才 × ${roundDisplay(materialRate)}/才${appliedWasteRate>0?` × ${roundDisplay(1+appliedWasteRate)}（耗損 ${roundDisplay(appliedWasteRate*100)}%）`:''}`,
      amount:singleMaterialCost,
      type:'材料'
    });
  }else{
    breakdown.push({label:`材料費暫以 0 計｜${materialRateSource}`,amount:0,type:'材料'});
  }

  processTags.forEach((rawTag)=>{
    const canonical=canonicalProcessTag(rawTag);
    const display=processTagDisplayLabel(canonical)||canonical||rawTag;
    if(unifiedIncludedProcess(canonical))return;
    if(hasBending&&(canonical==='導C角'||display==='導C角')){
      breakdown.push({label:'導C角｜折彎已包含，不重複計費',amount:0,type:'加工'});
      return;
    }
    const resolved=unifiedSpecialProcessRule(canonical);
    if(!resolved){
      missing.push(display);
      return;
    }
    const rule=resolved.rule;
    const stagePrice=toNumber(rule.price);
    let unitAmount=0;
    let label='';
    if(rule.type==='每才'){
    ...
    }else if(rule.type==='每孔'){
      const holes=Math.max(1,toNumber(item?.holes)||unifiedHoleCount(item)||1);
      unitAmount=stagePrice*holes;
      label=`${display}｜${holes}孔 × ${roundDisplay(stagePrice)}/孔`;
    }
    if(unitAmount>0){
      singleProcessCost+=unitAmount;
      breakdown.push({label,amount:unitAmount,type:'加工'});
    }
  });

  const minimumUnitPrice=180;
  const singleRawTotal=singleMaterialCost+singleProcessCost;
  const singleBeforeAdjust=Math.max(minimumUnitPrice,singleRawTotal);
  const minimumTopUp=Math.max(0,minimumUnitPrice-singleRawTotal);
  if(minimumTopUp>0)breakdown.push({label:`單件最低消費補足至 ${roundDisplay(minimumUnitPrice)}`,amount:minimumTopUp,type:'最低消費'});

  const quantityDiscount=qty>=10?0.85:qty>=5?0.90:1;
  let irregularShapeMultiplier=unifiedHasIrregularShape(processTags,item)?IRREGULAR_SHAPE_MULTIPLIER:1;
  const correctionFactor=bundleDiscountFactor(processTags,exactCai,aspectRatio);
  const unitBeforeDiscount=singleBeforeAdjust;
  const unitAfterQuantity=unitBeforeDiscount*quantityDiscount;
  const unitAfterIrregular=unitAfterQuantity*irregularShapeMultiplier;
  const unitPrice=Math.max(0,unitAfterIrregular*correctionFactor);
  const lineTotal=unitPrice*qty;

  if(quantityDiscount<1)breakdown.push({
    label:`數量折扣｜${qty}件 × ${roundDisplay(quantityDiscount*100)}%`,
    amount:-unitBeforeDiscount*(1-quantityDiscount),
    type:'數量折扣'
  });
  if(irregularShapeMultiplier>1)breakdown.push({
    label:'異形切割加成 ×1.3',
    amount:unitAfterQuantity*(irregularShapeMultiplier-1),
    type:'異形加成'
  });
  if(correctionFactor<1)breakdown.push({
    label:`廠商包價修正 ×${roundDisplay(correctionFactor)}`,
    amount:-unitAfterIrregular*(1-correctionFactor),
    type:'修正'
  });

  const formula=`max(${roundDisplay(singleMaterialCost)} + ${roundDisplay(singleProcessCost)}, ${roundDisplay(minimumUnitPrice)}) × ${quantityDiscount} × ${irregularShapeMultiplier} × ${correctionFactor} = ${roundDisplay(unitPrice)}/件`;
  const source='材料精確才數基價＋加工費逐項疊加＋情境修正';
  const confidence=Math.max(35,Math.min(94,90-missing.length*10-(item.estimateFailReason?20:0)));
  const details=`${sizeTier}${isLongStrip?'｜長條形件':''}｜${formula}｜${scenarioLabel}`;

  item.calculationFormula=formula;
  item.confidenceSource=source;
  item.costBreakdown=breakdown;
  item.missingProcessTags=missing;
  item.partialEstimate=missing.length>0||Boolean(item.estimateFailReason);
  item.priceFingerprint=[material.key,sizeTier,isLongStrip?'長條':'一般',correctionFactor,...processTags].filter(Boolean).join('｜');

  return{
    unitPrice:unitPrice>0?unitPrice:minimumUnitPrice*correctionFactor,
    materialUnitPrice:singleMaterialCost*quantityDiscount*irregularShapeMultiplier*correctionFactor,
    processUnitPrice:singleProcessCost*quantityDiscount*irregularShapeMultiplier*correctionFactor,
    materialLineTotal:singleMaterialCost*qty,
    processLineTotal:singleProcessCost*qty,
    source:`統一公式估價｜${source}｜${scenarioLabel}${item.estimateFailReason?`｜${item.estimateFailReason}`:''}${missing.length?`｜待補：${missing.join('、')}`:''}`,
    confidence,
    details,
    calculationFormula:formula,
    confidenceSource:source,
...
function estimateRangeFromCost(baseCost,orderType){
  ...
  switch(orderType){
    ...
    // 缺少製程價格時不應偷偷把「基準價」膨脹 25%；基準維持已算到的金額，風險只反映在保守上限。
    case'newProcess':return{low:value,base:value,high:value*1.50};
    case'prototype':
...
function recalculateEstimateItem(item){
  ...
  if(exactHistoricalMatch){
    ...
    applyAutomaticEstimatePricingUnit(item);item.usage=qty;item.usageManuallySet=true;
    item.wasteRate=0;item.marketAdjustment=0;item.marketManuallySet=true;
    item.costBreakdown=[{label:`${exactHistoricalMatch.approved?'已認證':'待認證'}精確歷史整件價`,amount:exactHistoricalMatch.unitPrice,type:'歷史'}];
    item.calculationFormula=`${qty}件 × ${Math.round(exactHistoricalMatch.unitPrice*100)/100}/件 = ${Math.round(exactHistoricalMatch.unitPrice*qty*100)/100}`;
    item.confidenceSource=`同品號／同尺寸／同材質厚度的${exactHistoricalMatch.approved?'已認證':'實際'}歷史報價`;
    item.similarHistoryRecords=exactHistoricalMatch.records||item.similarHistoryRecords;
  }else if(productionMatch&&productionMatch.score>=80){
  ...
  if(useManualPrice){
    item.priceId='';item.usage=qty;item.usageManuallySet=true;item.costBreakdown=[{label:'文件／人工單價',amount:toNumber(item.baseUnitPrice),type:'人工'}];
  ...
  const confidence=useManualPrice?Math.max(20,Math.min(98,toNumber(item.manualPriceConfidence)||75))
    :item.historicalPriceApplied&&exactHistoricalMatch?exactHistoricalMatch.confidence
...
function renderEstimateDraft(){
  ...
  recalculateAllEstimateItems();
  const workspace=$('estimateWorkspacePanel');
  if(workspace)workspace.hidden=!state.estimateDraftItems.length;
  ...
  if(!state.estimateDraftItems.length){
    $('estimateItemRows').innerHTML=`
      <div class="estimateEmptyState">
        ...
        <h3>尚未加入估價品項</h3>
        <p>上傳圖片或 PDF，按「AI 掃描並帶入試算」；也可以手動新增一筆。</p>
  ...
  state.estimateDraftItems.forEach((item)=>{ if(item.aiImported)refreshEstimateAiMissingFields(item); });
  $('estimateItemRows').innerHTML=state.estimateDraftItems.map((item,index)=>{
    const aiMissing=new Set(item.aiMissingFields||[]);
    const hasCriticalAiMissing=aiMissing.has('material')||aiMissing.has('dimension');
    const needsAiReview=Boolean(item._aiConfidenceLow || (item._missingFields||[]).length || (item.aiImported && toNumber(item.confidenceScore)<60));
    const priceMeta=estimatePriceStateMeta(item);
    ...
    const historyAverage=estimateHistoryAverage(item);
    const longItem=toNumber(item.widthMm)>600||toNumber(item.heightMm)>600;
    const hasQtyDiscount=toNumber(item.qty)>=5;
    ...
    const editOpen=needsAiReview||!String(item.name||'').trim()||!String(item.material||'').trim();
    const aspectRatio=(toNumber(item.widthMm)>0&&toNumber(item.heightMm)>0)
      ?Math.max(item.widthMm,item.heightMm)/Math.min(item.widthMm,item.heightMm)
      :0;
    ...
    return `<article class="estimateQuickCard estimateThreeLayerCard ${aiMissing.size?'hasAiMissing':''} ${hasCriticalAiMissing?'error-highlight':''} ${needsAiReview?'ai-review-card':''}" data-estimate-index="${index}">
      ...
      <header class="estimateSummaryLayer">
        <div class="estimateSummaryMain">
          ...
          <strong class="estimateSummaryName">${escapeHTML(item.name||'尚未命名')}</strong>
          <small class="estimateSummarySpec">${escapeHTML(estimateSummarySpec(item))}</small>
          <div class="estimateSummaryPills" data-estimate-summary-pills>${estimateProcessPillsHtml(tags)}</div>
          <div class="estimateSummaryTags">
            <span class="estimatePriceState ${priceMeta.className}">${escapeHTML(priceMeta.label)}</span>
            <span class="confidenceSemanticBadge ${confidence.className}" title="AI／公式信心 ${confidence.score}%">${escapeHTML(confidence.label)}</span>
            ${hasQtyDiscount?'<span class="estimateDiscountTag">含多件折扣</span>':''}
            ${longItem?'<span class="estimateLongItemTag">🚨 超長件</span>':''}
            ${needsAiReview?'<span class="aiReviewBadge" data-ai-review-badge>⚠️ 需人工確認</span>':''}
          </div>
      ...
      <details class="estimateEditLayer" data-estimate-edit-details ${editOpen?'open':''}>
        <summary>編輯品項與製程</summary>
        <div class="estimateQuickFields">
          <label class="estimateFieldName ${aiMissing.has('name')?'aiMissingField':''}"><span>品項名稱</span><input class="tableInput" data-estimate-field="name" value="${escapeHTML(item.name)}" placeholder="品項名稱"></label>
          <label class="${aiMissing.has('material')?'aiMissingField error-highlight':''}"><span>材質</span><select class="tableInput materialInput" data-estimate-field="material">${standardMaterialOptionsHtml(item.material)}</select></label>
          <label class="${aiMissing.has('thickness')?'aiMissingField':''}"><span>厚度</span><input class="tableInput" data-estimate-field="thickness" value="${escapeHTML(item.thickness)}" placeholder="3mm"></label>
          <div class="estimateFieldDimensions ${aiMissing.has('dimension')?'aiMissingField error-highlight':''}" role="group" aria-label="規格尺寸，高乘寬">
            <input type="hidden" data-estimate-field="spec" value="${escapeHTML(item.spec)}">
            <label><span>H（高）mm</span><input class="tableInput numberInput" data-estimate-field="heightMm" type="number" min="0" step="0.1" value="${item.heightMm||''}" placeholder="300"></label>
            <span class="estimateDimensionTimes" aria-hidden="true">×</span>
            <label><span>W（寬）mm</span><input class="tableInput numberInput" data-estimate-field="widthMm" type="number" min="0" step="0.1" value="${item.widthMm||''}" placeholder="500"></label>
          </div>
          ...
          <input type="hidden" data-estimate-field="unit" value="${escapeHTML(item.unit)}">
          <div class="estimateAutoUnit" aria-label="系統自動計價單位"><span>自動計價單位</span><strong>${escapeHTML(item.pricingUnit||item.unit||automaticEstimatePricingUnit(item))}</strong></div>
          <label class="${aiMissing.has('baseUnitPrice')?'aiMissingField':''}"><span>單價</span><input class="tableInput numberInput manualPriceInput" data-estimate-field="baseUnitPrice" type="number" min="0" step="0.01" value="${roundDisplay(item.baseUnitPrice)}" placeholder="待配對"></label>
        ...
        ${aiMissing.size?`<div class="estimateAiMissingBanner">AI 待補欄位：${escapeHTML([...aiMissing].map(estimateAiMissingLabel).join('、'))}。請補填紅色欄位後再確認估價。</div>`:''}
      </details>

      <details class="estimateCalculationLayer">
        <summary>查看計算明細</summary>
        <div class="estimateCalculationContent">
          <div class="estimateCostBreakdown ${item.missingProcessTags?.length?'hasMissing':''}" data-estimate-breakdown>${renderEstimateBreakdownHtml(item)}</div>
          <div class="estimateRangeGrid" data-estimate-range>
            <div><span>樂觀</span><strong>${money(item.optimisticCost)}</strong></div>
            <div class="current"><span>基準</span><strong>${money(item.baselineCost)}</strong></div>
            <div><span>保守</span><strong>${money(item.conservativeCost)}</strong></div>
          </div>
          <div class="estimateHistoryAverage" data-estimate-history-average>${historyAverage?`歷史報價均價參考：<strong>${money(historyAverage.average)}/件</strong>（${historyAverage.count} 筆）`:'尚無足夠相似歷史報價'}</div>
          <div class="estimateFormulaDetail" data-estimate-formula-detail>${renderEstimateFormulaHtml(item)}</div>
        </div>
      </details>

      <details class="estimateItemAdvanced">
        <summary>進階設定：公司價格、用量損耗與人工調整</summary>
        <div class="estimateAdvancedGrid">
          ...
          <section class="estimateFieldGroup">
            <h4>印刷與降本條件</h4>
            <div class="checkRow"><label><input type="checkbox" data-estimate-field="allowMaterialOptimization" ${item.allowMaterialOptimization!=='否'?'checked':''}>允許材質優化</label><label><input type="checkbox" data-estimate-field="allowPrintOptimization" ${item.allowPrintOptimization!=='否'?'checked':''}>允許印刷優化</label></div>
      ...
      <section class="estimateDeveloperLayer" aria-label="開發者除錯資料">
        <div class="estimateDeveloperHead"><strong>開發者資料</strong><span>僅供公式校驗與除錯</span></div>
        <div class="estimateDimensionSummary ${item.dimensionStatus==='待確認'?'warning':''}" data-estimate-dimension-summary>
          <span><strong>繁中：</strong>${escapeHTML(item.normalizedName||item.name||'—')}</span>
          <span><strong>尺寸：</strong>${isLightStripItem(item)?(item.lightStripTotalLengthM?`${roundDisplay(item.lightStripTotalLengthM)} m（整列）`:'待確認長度'):(item.widthMm&&item.heightMm?`H ${roundDisplay(item.heightMm)} × W ${roundDisplay(item.widthMm)} mm`:'待確認')}</span>
          ...
          <span><strong>長寬比：</strong>${aspectRatio?roundDisplay(aspectRatio):'—'}</span>
        ...
        <div class="estimateDeveloperFields">
          <label><span>品項代碼</span><input class="tableInput" data-estimate-field="code" value="${escapeHTML(item.code)}"></label>
          <label><span>原始品名</span><input class="tableInput" data-estimate-field="originalName" value="${escapeHTML(item.originalName)}"></label>
          <label><span>原始規格</span><input class="tableInput" data-estimate-field="originalSpec" value="${escapeHTML(item.originalSpec)}"></label>
          ${isLightStripItem(item)?`<label><span>整列長度 m</span><input class="tableInput numberInput" data-estimate-field="usage" type="number" min="0" step="0.001" value="${roundDisplay(item.usage||item.lightStripTotalLengthM||0)}"></label>`:`<label><span>手動計價才數</span><input class="tableInput numberInput" data-estimate-field="billingTsai" type="number" min="0" step="0.01" value="${roundDisplay(item.billingTsai)}"></label>`}
        </div>
      </section>
    </article>`;
  }).join('');
...
function formatEstimateDimensionSpec(widthMm,heightMm){
...
function renderEstimateBreakdownHtml(item){
  return`${(item.costBreakdown||[]).length
    ?item.costBreakdown.map((part)=>`<span class="breakdownChip"><b>${escapeHTML(part.label)}</b> ${money(part.amount)}</span>`).join('')
    :'<span class="breakdownMuted">尚無可用分項價格</span>'}${item.missingProcessTags?.length?`<span class="breakdownMissing">待補：${escapeHTML(item.missingProcessTags.join('、'))}</span>`:''}`;
}

function renderEstimateFormulaHtml(item){
  return`<span><strong>計算方式：</strong>${escapeHTML(item.calculationFormula||'尚無公式')}</span>
    <span><strong>信心來源：</strong>${escapeHTML(item.confidenceSource||item.priceSource||'待確認')}</span>
    ${(item.similarHistoryRecords||[]).length?`<div class="similarHistoryList"><strong>相似歷史報價 ${Math.min(5,item.similarHistoryRecords.length)} 筆</strong>${item.similarHistoryRecords.slice(0,5).map((row)=>`<span>${escapeHTML(row.itemName||'未命名')}｜${escapeHTML(row.material||'')} ${escapeHTML(row.thicknessMm?`${row.thicknessMm}mm`:'')}｜${row.widthMm&&row.heightMm?`${roundDisplay(row.widthMm)}×${roundDisplay(row.heightMm)}mm｜`:''}${money(row.unitPrice)}</span>`).join('')}</div>`:''}`;
}

function renderEstimateDimensionSummaryHtml(item){
  const processingDisplay=item.processingLevelResolved&&item.processingLevelResolved!=='NONE'
    ?`${item.processingLevelResolved}｜${item.processingLevelSource||'自動判斷'}`
    :(item.processingLevelResolved==='NONE'?'不計加工':'尚未判斷');
  const thicknessFallback=resolveInternalRuleThickness(item.material||item.name,numericThickness(item.thickness));
  const thicknessFallbackWarning=item.internalRuleApplied&&thicknessFallback.usedFallback?thicknessFallback.warning:'';
  const aspectRatio=(toNumber(item.widthMm)>0&&toNumber(item.heightMm)>0)?Math.max(item.widthMm,item.heightMm)/Math.min(item.widthMm,item.heightMm):0;
  return`<span><strong>繁中：</strong>${escapeHTML(item.normalizedName||item.name||'—')}</span>
    <span><strong>尺寸：</strong>${isLightStripItem(item)?(item.lightStripTotalLengthM?`${roundDisplay(item.lightStripTotalLengthM)} m（整列）`:'待確認長度'):(item.widthMm&&item.heightMm?`H ${roundDisplay(item.heightMm)} × W ${roundDisplay(item.widthMm)} mm`:'待確認')}</span>
    <span><strong>級距：</strong>${escapeHTML(item.sizeTier||'待確認')}</span>
    ${isLightStripItem(item)?`<span><strong>單件長度：</strong>${roundDisplay(item.lightStripSingleLengthM||0)} m</span><span><strong>整列長度：</strong>${roundDisplay(item.lightStripTotalLengthM||0)} m</span>`:`<span><strong>精確才數：</strong>${roundDisplay(item.totalExactTsai||0)}</span><span><strong>整列建議計價才數：</strong>${roundDisplay(item.billingTsai||0)}</span>`}
    <span><strong>長寬比：</strong>${aspectRatio?roundDisplay(aspectRatio):'—'}</span>
...
function handleEstimateItemInput(event){
  ...
  if(field==='processSummary') applyEstimateProcessSummary(item,item.processSummary);
  ...
  if(['name','material','thickness','spec','originalName','originalMaterial','originalSpec','widthMm','heightMm','qty'].includes(field)){
    ...
    if(['widthMm','heightMm'].includes(field)){
      ...
      item.dimensions=(toNumber(item.widthMm)>0&&toNumber(item.heightMm)>0)
        ...
        : [];
      const dimensionSpec=formatEstimateDimensionSpec(item.widthMm,item.heightMm);
      if(dimensionSpec)item.spec=dimensionSpec;
    }
  ...
  recalculateEstimateItem(item);
  refreshEstimateAiMissingFields(item);
  updateEstimateAiReviewState(row,item);
  updateEstimateRowSummary(row,item);
  updateEstimateTotals();
  updateEstimateQuickMetrics();
  if(event.type==='change' && ['priceId','baseUnitPrice','material','thickness','spec','unit','originalName','originalSpec','widthMm','heightMm','billingTsai','processingLevel','printMethod','printSide','whiteInk','specialEffect','constraints','processSummary'].includes(field)) renderEstimateDraft();
}
...
function updateEstimateRowSummary(row,item){
  ...
  if(title)title.textContent=item.name||'尚未命名';
  const spec=row.querySelector('.estimateSummarySpec');
  if(spec)spec.textContent=estimateSummarySpec(item);
  const pills=row.querySelector('[data-estimate-summary-pills]');
  if(pills)pills.innerHTML=estimateProcessPillsHtml(item.processSummary||estimateProcessSummaryText(item));
  const priceState=row.querySelector('.estimatePriceState');
  if(priceState){
    const meta=estimatePriceStateMeta(item);
    priceState.textContent=meta.label;
    priceState.className=`estimatePriceState ${meta.className}`;
  }
  const confidenceBadge=row.querySelector('.confidenceSemanticBadge');
  if(confidenceBadge){confidenceBadge.className=`confidenceSemanticBadge ${confidence.className}`;confidenceBadge.textContent=confidence.label;confidenceBadge.title=`AI／公式信心 ${confidence.score}%`;}
  const breakdown=row.querySelector('[data-estimate-breakdown]');
  if(breakdown){
    breakdown.classList.toggle('hasMissing',Boolean(item.missingProcessTags?.length));
    breakdown.innerHTML=renderEstimateBreakdownHtml(item);
  }
  const formula=row.querySelector('[data-estimate-formula-detail]');
  if(formula)formula.innerHTML=renderEstimateFormulaHtml(item);
  const range=row.querySelector('[data-estimate-range]');
  if(range)range.innerHTML=`<div><span>樂觀</span><strong>${money(item.optimisticCost)}</strong></div><div class="current"><span>基準</span><strong>${money(item.baselineCost)}</strong></div><div><span>保守</span><strong>${money(item.conservativeCost)}</strong></div>`;
  const history=row.querySelector('[data-estimate-history-average]');
  if(history){const meta=estimateHistoryAverage(item);history.innerHTML=meta?`歷史報價均價參考：<strong>${money(meta.average)}/件</strong>（${meta.count} 筆）`:'尚無足夠相似歷史報價';}
  const dimension=row.querySelector('[data-estimate-dimension-summary]');
  if(dimension){
    dimension.classList.toggle('warning',item.dimensionStatus==='待確認');
    dimension.innerHTML=renderEstimateDimensionSummaryHtml(item);
  }
  const status=row.querySelector('[data-estimate-auto-status]');
  if(status){
    status.textContent=`已自動重新計算 ${new Date().toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}`;
  ...
  const billingTsai=row.querySelector('input[data-estimate-field="billingTsai"]');
  if(billingTsai&&document.activeElement!==billingTsai)billingTsai.value=roundDisplay(item.billingTsai);
  row.querySelectorAll('input[data-estimate-field="widthMm"]').forEach((input)=>{if(document.activeElement!==input)input.value=item.widthMm||'';});
  row.querySelectorAll('input[data-estimate-field="heightMm"]').forEach((input)=>{if(document.activeElement!==input)input.value=item.heightMm||'';});
  row.querySelectorAll('input[data-estimate-field="spec"]').forEach((input)=>{input.value=item.spec||'';});
  row.querySelectorAll('input[data-estimate-field="processSummary"]').forEach((input)=>{input.value=item.processSummary||estimateProcessSummaryText(item);});
}
...
function handleEstimateItemClick(event){
  ...
  const openProcess=event.target.closest('[data-estimate-open-process]');
  if(openProcess){
    const card=openProcess.closest('[data-estimate-index]');
    ...
    if(details){
      ...
      requestAnimationFrame(()=>{
        ...
        panel?.querySelector('.processTagButton')?.focus({preventScroll:true});
      });
    }
    return;
  }
...
function parseEstimateVersionNumber(value){
  const match=String(value||'').match(/(\d+)/);
  return match?Number(match[1]):0;
}

function nextEstimateVersionForContext(name,machineId){
  const targetName=norm(name||'');
  const matches=(state.estimateProjects||[]).filter((project)=>{
    if(machineId&&targetName)return project.machineId===machineId&&norm(project.name)===targetName;
    if(machineId)return project.machineId===machineId;
    return targetName&&norm(project.name)===targetName;
  });
  const maxVersion=matches.reduce((max,project)=>Math.max(max,parseEstimateVersionNumber(project.version)),0);
...
function applyQuickEstimateDefaults(){
  ...
  const basis=$('estimatePriceBasis');
  const orderType=$('estimateOrderType');
  if(version&&!String(version.value||'').trim())version.value='v1';
...
function historicalSaveInsight(item){
  if(!item)return{average:0,count:0,similarity:0};
  const probe={...emptyEstimateItem(),...item};
  const parsed=parseDimensions(probe.spec||'');
  probe.widthMm=toNumber(probe.widthMm)||parsed.widthMm;
  probe.heightMm=toNumber(probe.heightMm)||parsed.heightMm;
  const entries=similarApprovedHistoricalPricesForItem(probe,20).filter((entry)=>toNumber(entry.row?.unitPrice)>0);
  if(!entries.length)return{average:0,count:0,similarity:0};
  return{
    average:entries.reduce((sum,entry)=>sum+toNumber(entry.row.unitPrice),0)/entries.length,
    count:entries.length,
    similarity:Math.max(0,Math.min(100,Math.round(entries[0].score||0))),
  };
}

function savedRecordFeedback(item,total,itemCount=1){
  const label=[item?.name||'未命名品項',item?.material,item?.thickness].filter(Boolean).join(' ');
  const dimensions=toNumber(item?.widthMm)>0&&toNumber(item?.heightMm)>0?` ${roundDisplay(item.widthMm)}×${roundDisplay(item.heightMm)}mm`:'';
  const insight=historicalSaveInsight(item);
  const current=money(total);
  const history=insight.count?`約 ${money(insight.average)}/件（相似度 ${insight.similarity}%）`:'尚無足夠相似紀錄';
  return `✅ 已儲存！${label}${dimensions} = ${current}${itemCount>1?`（本次共 ${itemCount} 筆）`:''}\n📊 歷史均價參考：${history}`;
}

function showSaveSuccessToast(savedItem){
  const payload=savedItem&&savedItem.item?savedItem:{item:savedItem};
  const item=payload.item||{};
  const total=toNumber(payload.total??payload.baselineTotal??item.baselineCost??item.price);
  const itemCount=Math.max(1,toNumber(payload.itemCount)||1);
  const toast=$('saveSuccessToast');
  if(!toast){
    showNotice(savedRecordFeedback(item,total,itemCount),'success');
    return;
  }
  const label=[item.name||'未命名品項',item.material,item.thickness].filter(Boolean).join(' ');
  const parsed=parseDimensions(item.spec||'');
  const width=toNumber(item.widthMm)||parsed.widthMm;
  const height=toNumber(item.heightMm)||parsed.heightMm;
  const dimensions=width>0&&height>0?` ${roundDisplay(width)}×${roundDisplay(height)}mm`:'';
  const insight=historicalSaveInsight({...item,widthMm:width,heightMm:height});
  const headline=`✅ 已儲存！${label}${dimensions} = ${money(total)}${itemCount>1?`（本次共 ${itemCount} 筆）`:''}`;
  const history=insight.count?`📊 歷史均價參考：約 ${money(insight.average)}/件（相似度 ${insight.similarity}%）`:'';
  toast.innerHTML=`<strong>${escapeHTML(headline)}</strong>${history?`<span>${escapeHTML(history)}</span>`:''}`;
  toast.hidden=false;
  requestAnimationFrame(()=>toast.classList.add('show'));
  clearTimeout(saveSuccessToastTimer);
  saveSuccessToastTimer=setTimeout(()=>{
    toast.classList.remove('show');
    setTimeout(()=>{toast.hidden=true;},220);
  },5000);
}

function resetEstimateEditor(){state.currentEstimateId='';state.estimateDraftItems=[];clearEstimateTextInput();$('estimateSaveSettingsForm')?.reset();$('estimateId').value='';$('estimateTargetType').value='new';$('estimatePriceBasis').value='sampleTwd';$('estimateOrderType').value='auto';$('estimateVersion').value='v1';$('estimateDate').value=todayValue();$('estimateName').value=defaultEstimateProjectName($('estimateDate').value);$('estimateStatus').value='草稿';if($('estimateSyncCost'))$('estimateSyncCost').checked=false;clearEstimateSourceDocument();updateEstimateTargetMode();renderEstimateDraft();}
...
let estimateSaveSettingsResolver=null;
function openEstimateSaveSettingsDialog(){
  const dialog=$('estimateSaveSettingsDialog');
  if(!dialog)return Promise.resolve(true);
  if(dialog.open)return Promise.resolve(false);
  return new Promise((resolve)=>{
    estimateSaveSettingsResolver=resolve;
    dialog.showModal();
    setTimeout(()=>$('estimateTargetType')?.focus(),0);
  });
}
function closeEstimateSaveSettingsDialog(confirmed=false){
  const dialog=$('estimateSaveSettingsDialog');
  if(dialog?.open)dialog.close();
  const resolve=estimateSaveSettingsResolver;
  estimateSaveSettingsResolver=null;
  if(resolve)resolve(Boolean(confirmed));
}
function handleEstimateSaveSettingsSubmit(event){
  event.preventDefault();
  closeEstimateSaveSettingsDialog(true);
}

async function saveCurrentEstimate(options={}){
  if(!options.skipSettings){
    const confirmed=await openEstimateSaveSettingsDialog();
  ...
  try{
    ...
    if(!state.currentEstimateId){
      ...
      $('estimateVersion').value=autoVersion;
    }
    const response=await secureApiRequest({action:'saveEstimate',project:{id:state.currentEstimateId,machineId,name,version:$('estimateVersion').value.trim()||'v1',date:$('estimateDate').value,status:$('estimateStatus').value||'草稿',note:$('estimateNote').value.trim()},items:valid},{timeoutMs:120000});
